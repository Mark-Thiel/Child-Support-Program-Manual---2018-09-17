...
function writeBook(nIdx)
{
	...
	if(bLocal)
	{
		if (!isBookEmpty(nIdx))
		{
			...
			if(sURL!="")
				sBookRef = sURL;
sHTML+="parent><p style=\"margin-left: 16px; text-indent: -22px;\"><span";

if(!sName.match(" "))
  sHTML+= " style=\"white-space: nowrap;\"";

sHTML+="><a id=\""+getBookId(nIdx)+"\" href=\""+sBookRef+"\" onfocus=\"markBook("+nIdx+");\" onclick=\"";
			if(gbSafari3)
...
sHTML+= "><a id=\""+getItemId(nIdx)+"\" onfocus =\"markItem("+nIdx+");\" onclick=\"markItem("+nIdx+");\""
	var sAltString="";
	if(nItemType&2)
		sAltString="Page";
	else
	...
	sHTML+="href=\""+_textToHtml_nonbsp(getItemURL(nIdx))+"\" title=\""+sName+"\"><img alt=\""+sAltString+"\" src=\""+sIcon+"\" border=0 align=\"absmiddle\">&nbsp;"+sName+"</a></span></p>";
	return sHTML;
}
